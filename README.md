...
Every core feature phase from `CLAUDE.md`'s roadmap is built and then some: authentication and household access (public self-registration creates a brand new household, gated on email verification; joining an *existing* household is still invite-only — role-based, Owner/Administrator/Member/Viewer — with optional TOTP two-factor authentication and active session management), manual accounts with full balance history, transactions (income/expense/transfers, split transactions, receipt/PDF attachments, tags, bulk editing, search/filter, CSV import/export, a global search across transactions/accounts/categories/tags), monthly budgets with user-defined sections and a guided review wizard (optionally nudged by an emailed, scoped, single-use link a few days before each month starts — Settings → Household), a full transaction rules engine (auto-categorize/tag/rename on creation, with a preview-and-confirm retroactive apply), recurring bills and subscriptions, in-app notifications, a dashboard with real charts (net worth trend, income vs. spending, spending by category) and a responsive mobile/tablet nav (installable to a phone's home screen as a standalone app via `public/manifest.json` — no separate native app needed), savings goals, a debt overview with snowball/avalanche payoff comparisons, a cash flow page, and a configurable reports page with CSV export and print-to-PDF. Phase 8 (production hardening — security headers, automated tests, backups, this documentation) is also in place; see `docs/security.md`'s "Known limitations" for what's intentionally deferred.
...
SMTP (via PHPMailer, `.env`'s `MAIL_*`) is available and used by budget planning reminders, household invitations, and registration email verification — see `.env.example` for Gmail/Workspace setup. Password resets haven't been switched over yet and still log their link instead of emailing it.

## First-time setup

Anyone can create their own household at `/register` (gated on confirming their email — see `docs/security.md`). To create *this* deployment's very first household from the command line instead (no email round-trip required):
