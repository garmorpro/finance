...
<body>
    <div class="app-shell">
        ...
        <div class="account-card" style="background-color: <?= View::e($accountColor) ?>;">
            ...
            <div class="min-w-0">
                <div class="text-2xl font-semibold truncate"><?= View::e($account['name']) ?></div>
                <?php if (!empty($account['institution_name'])): ?>
                    <div class="text-sm text-white/70 truncate"><?= View::e($account['institution_name']) ?></div>
                <?php endif; ?>
            </div>
            <div class="flex items-end justify-between mt-2">
                <span class="text-lg tracking-widest text-white/60" aria-hidden="true">&bull;&bull;&bull;&bull;</span>
                <div class="text-right">
                    <div class="text-[10px] font-semibold uppercase tracking-wide text-white/60">Current balance</div>
                    <div class="text-2xl font-semibold"><?= Money::format($account['current_balance']) ?></div>
                </div>
            </div>
        </div>
        ...
        <div class="card">
            ...
            <div class="mt-6">
                ...
                <table class="table-base">
                    <thead>
                        <tr><th>When</th><th>By</th><th>Change</th><th>Note</th></tr>
                    </thead>
                    <tbody>
                        <?php foreach ($history as $entry): ?>
                            <tr>
                                <td class="text-stone-500 dark:text-stone-400 whitespace-nowrap"><?= View::e($entry['created_at']) ?></td>
                                <td class="whitespace-nowrap"><?= View::e($entry['changed_by_name']) ?></td>
                                <td class="whitespace-nowrap">
                                    ...
                                    <span class="font-medium text-stone-900 dark:text-white"><?= Money::format($entry['new_balance']) ?></span>
                                </td>
        ...
        <form method="POST" action="/accounts/<?= (int) $account['id'] ?>" class="card space-y-6">
            <input type="hidden" name="csrf_token" value="<?= View::e($csrfToken) ?>">
            ...
            <div class="border-t border-stone-100 dark:border-stone-800 pt-6">
                <h3 class="text-xs font-bold uppercase tracking-wide text-stone-500 dark:text-stone-400 mb-3">Balances &amp; limits</h3>
            ...
            <div class="border-t border-stone-100 dark:border-stone-800 pt-6">
                <h3 class="text-xs font-bold uppercase tracking-wide text-stone-500 dark:text-stone-400 mb-3">Appearance</h3>
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        <label for="color" class="field-label">Card color</label>
